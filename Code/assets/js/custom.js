...
$("#modal-add-leaves").fireModal({
	...
	onFormReady: function(modal, form) {
	  // Update leave counts when the create form is shown
	  updateLeaveCounts();
	}
  });
    ...
    $.ajax({
        ...
        success: function(result) 
        {
        	...
        	if(result['error'] == false){
				...
				if(result['data'].users != '' && result['data'].users != null){
					$("#users").val(result['data'].users.split(','));
					$("#users").trigger("change");
				}
    ...
    $.ajax({
        ...
        success: function(result) 
        {
        	...
        	if(result['error'] == false){
				...
				var newArray = [];
				if(result['data'].permissions != '' && result['data'].permissions != null){
					var inputString = result['data'].permissions.replace(/[\[\]"]+/g, ''); // Remove brackets and double quotes
					newArray = inputString.split(',');
				}
				$("#permissions").val(newArray);
				$("#permissions").trigger("change");
				
				var newArray = [];
				if(result['data'].assigned_users != '' && result['data'].assigned_users != null){
					var inputString = result['data'].assigned_users.replace(/[\[\]"]+/g, ''); // Remove brackets and double quotes
					newArray = inputString.split(',');
				}
				$("#users").val(newArray);
				$("#users").trigger("change");
    ...
    $.ajax({
        ...
        success: function(result) 
        {
        	...
        	if(result['error'] == false){
				...
				$("#total_leaves").val(result['data'].total_leaves);
				$("#total_leaves").trigger("change");
...
$("#modal-edit-leaves-type").fireModal({
    ...
    $.ajax({
	    ...
	    success:function(result){
		    if(result['error'] == false){
				$('#leaves_type_list').bootstrapTable('refresh');
				modal.modal('hide');
		    }else{
		        modal.find('.modal-body').append('<div class="alert alert-danger">'+result['message']+'</div>');
		    }
