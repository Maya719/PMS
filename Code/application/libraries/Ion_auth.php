...
class Ion_auth
{
	...
	public function forgotten_password($identity)
	{
		...
		if ($user)
		{
			...
			if ($code)
			{
				...
				{
					$template_data = array();
					$template_data['RESET_PASSWORD_LINK'] = base_url('auth/reset_password/'.$code);
					$email_template = render_email_template('forgot_password', $template_data);
					
					if (send_mail($identity, $email_template[0]['subject'], $email_template[0]['message']))
					{
						$this->set_message('forgot_password_successful');
						return TRUE;
					}
	...
	public function is_client($id = FALSE)
	{
		...
		return $this->ion_auth_model->in_group($client_group, $id);
	}
